...
By: `Team T11-B3`      Since: `Aug 2017`      Licence: `MIT`
...
The detag mechanism is facilitated by the `DetagCommand` class which inherited from the `UndoableCommand` class. It allows for the removing of existing tag from multiple person, i.e. detag. The minimum number of person specified is 1 and the maximum number is the number of person on the filtered list.

Suppose the user to execute the command `detag` using the following code
----
detag 1, 2 t/friends
----

The `DetagCommandParser` class will check for the validity of the indices specified, i.e. within the range of the filtered list. Afterwards, the `LogicManager` invokes the method `deleteTag`, which removes the specific tag from the `Set` of tags from the specified persons, and update the storage.

[NOTE]
If one or more of the persons specified by the indices does not contain the specified tag to remove, the removal will not be executed even for those persons that have the tag.

[NOTE]
The current detag mechanism does not support multiple tags removal, i.e. remove 2 different tags in one command line.

==== Design Considerations
**Aspect:** Implementation of `DetagCommand` +
**Alternative 1 (current choice):** Directly removes the tag from `Set` of person(s) +
**Pros:** Easier and more specific command to remove tag, and could remove from multiple persons in one command +
**Cons:** Makes the tags of person skeptical for changes, and introducing of new additional command specifically for removal of tag may be not necessary +
...
**Cons:** Could not remove tag of multiple persons, and have to type in the tags that do not need to be remove +
// end::detag[]

=== FindTag mechanism

The FindTag mechanism is facilitated by the `FindTagCommand` class which inherits from `Command` class. It allows the user to find persons using tags. User can search using multiple tags too.

Suppose the user has just executed the `FindTagCommand` using `findtag friends`.

At first, the `execute()` function of the `LogicManager` class is called, which goes on to pass the command string into the `parseCommand()` function of the `AddressBookParser` class. This, in turn, determines that the `FindTagCommand` feature is being evoked and calls the `FindTagCommandParser` to parse the string and compare the tags. These are validated and formatted by the `TagContainsKeywordsPredicate` and return as a `TagContainsKeywordsPredicate` object to be used to call the `FindTagCommand` class. The `LogicManager` then calls the `execute()` in the `FindTagCommand` class which creates and return a `CommandResult` object.
...
Suppose the user has just executed the `addtask` command using `addtask CS assignment from 15-10-2017 to 20-10-2017`.
...
The following activity diagram summarize what happens when a user executes a new command:

image::AddTaskCommand.png[width="800", height="500"]
...
**Cons:** Date not stored the correct format. +
**Alternative 2:** Use java.time.LocalDate class. +
**Pros:** Dates are in correct format. +
**Cons:** LocalDate cannot be initialized when no input is given. Extra code needed to make dates optional.
...
|`* * *` |user |find a person by address, birthday, phone, tags |locate details of persons when name is forgetten

|`* * *` |user |add a new person leaving out any details other than name |add a person that I am not familar with
...
|`* * *` |user |add contacts while leaving one or more fields blank, except for the name |save contacts without having to know every detail

|`* * *` |user |search my contacts by name, number, tag or address |find people more easily
...
=== Use Case: Remove tag of multiple person
...
=== Use Case: Find the person using tags
